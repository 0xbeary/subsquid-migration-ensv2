...
type EventHandlers<T extends ABI> = {
    [K in keyof T['events'] as `${typeof HANDLER_PREFIX}${string & K}`]?: EventHandler<ReturnType<T['events'][K]['decode']>>
}
...
function getHandler<T extends ABI, K extends keyof EventHandlers<T>>(
    handlers: EventHandlers<T>,
    key: K,
): EventHandlers<T>[K] {
    return handlers[key]
...
export class Mapper {
    ...
    add<T extends ABI>(mapping: Mapping<T>) {
        ...
        for (const handlerName in mapping.handlers) {
            ...
            const handler = getHandler(mapping.handlers, handlerName) as EventHandler<any>
            if (handler) {
